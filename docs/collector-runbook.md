...
When champion-page prewarming is enabled, worker startup gives user-facing readiness priority over collection: it warms every canonical page for the configured current patch and the mature fallback patch with bounded concurrency, leaves a short readiness window for deployment certification, then begins Riot collection. The broader role and common-matchup pass remains in the analytics scheduler as secondary work. Cold item/loadout fallbacks prune timeline scans to the selected champion's match IDs so a missing read-model partition cannot trigger a whole-history scan.
