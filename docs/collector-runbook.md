...
Patch rollover maintenance is deliberately paced. Backfills first identify missing match IDs, copy normalized participant data where available, and restrict the remaining raw JSON work to 250-match batches. The helper also runs `patchctl` with conservative ClickHouse query settings by default: `WINRIFT_PATCHCTL_CLICKHOUSE_MAX_THREADS=2`, `WINRIFT_PATCHCTL_CLICKHOUSE_MAX_MEMORY_MB=2048`, `WINRIFT_PATCHCTL_CLICKHOUSE_MAX_OPEN_CONNS=2`, and `WINRIFT_PATCHCTL_CLICKHOUSE_MAX_EXECUTION_TIME_SECONDS=1800`. `patchctl` waits between archive phases when one-minute load is above `WINRIFT_PATCHCTL_MAX_LOAD_1M` (default: host CPU count) or available memory is below `WINRIFT_PATCHCTL_MIN_AVAILABLE_MEMORY_MB` (default: 1024). Tune these only if the server has more room or if maintenance needs to be even gentler.
