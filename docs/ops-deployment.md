...
The helper prompts for the new Riot key, shows the full pasted key back for visual verification, and requires typing `YES` before it updates `/srv/winrift/.env`. After confirmation, it clears the shared marker, starts ClickHouse if needed, checks Data Dragon for the latest patch bucket, archives and prunes raw data outside the new retention window, updates `COLLECTOR_CURRENT_PATCH` if Riot has advanced, recreates the API, ensures the monitor is running, and starts the worker. It is designed for laptop and phone SSH workflows. Both API and worker also clear the marker on startup, but the helper avoids manual env editing and Docker command memorization. If Riot briefly returns `401` while a refreshed development key propagates, the helper clears the marker and retries worker startup before failing. If the short command is not on the shell `PATH`, use `/srv/winrift/refresh-riot-key`. Use `refresh-riot-key --hide-key` if you want a masked preview instead of full-key echo, or `refresh-riot-key --no-patch-rollover` if you intentionally want to refresh only the key.
