...
After the API, monitor, and optional worker are recreated, the deploy workflow runs a post-deploy smoke pass. It verifies container state, `/api/health`, leaderboard/profile cache-hit behavior, and the worker refresh-status JSON when present. With the worker enabled, startup first prewarms every canonical current-patch champion page plus the mature-fallback pages for champions whose current guide is empty. The API bulk-hydrates those canonical bundles from the persistent cache into its own in-process cache and refreshes that hydration every 30 minutes. Deployment waits for the worker readiness markers and the API memory-hydration marker, then strictly audits every canonical champion page at a 500 ms default ceiling. A deploy fails if any page is uncached, incomplete, slow, or unable to provide a mature fallback when current-patch guide data is empty.
