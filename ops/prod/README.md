...
After the API, monitor, and optional worker are recreated, the deploy workflow runs a post-deploy smoke pass. It verifies container state, `/api/health`, leaderboard/profile cache-hit behavior, and the worker refresh-status JSON when present. With the worker enabled, startup first prewarms every canonical champion page for the current patch and its mature fallback before Riot collection begins. Deployment waits for those two explicit readiness markers and strictly audits every canonical champion page at a 500 ms default ceiling. A deploy fails if any page is uncached, incomplete, slow, or unable to provide a mature fallback when current-patch guide data is empty.
